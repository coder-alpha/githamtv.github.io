...
def findPHP(data, streamId):
    ...
    regex = "document.write\('.*?src=['\"]*(.*?f*id\s*\+'\.html).*?['\" ]*.*?\)"
    html = regexUtils.findall(data, regex)
    if html:
        return re.sub(r"\'\+\s*f*id\s*\+\'", "%s" % streamId,html[0])
...
def findVCods(data):
    ...
    vcods = regexUtils.findall(data, regex)
    if vcods:
        return vcods[0]
...
def findVideoFrameLink(page, data):
    ...
    iframes = regexUtils.findall(data, "(frame(?![^>]*cbox\.ws)(?![^>]*chat\.php)(?![^>]*ad122m)(?![^>]*capacanal)(?![^>]*programacion)[^>]*\sheight\s*=\s*[\"']*([\%\d]+)(?:px)?[\"']*[^>]*>)")
