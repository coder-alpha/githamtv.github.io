...
class Main:
    ...
    def downloadVideo(self, url, title):
        ...
        title = getKeyboard(default = fu.cleanFilename(title),heading='PGLiveTV')
    ...
    def downloadCustomModule(self):
        ...
        if success == True:            
            # refresh container if PGLiveTV is active
            currContainer = xbmcUtils.getContainerFolderPath()
            common.showNotification('PGLiveTV', 'Download successful', 1000)
            if currContainer.startswith(self.base):                
                xbmc.executebuiltin('Container.Refresh()')
            return True
        elif success == False:        
            common.showNotification('PGLiveTV', 'Download failed', 1000)
        return False
        ...
        if definedIn:
            ...
            if definedIn.endswith('favourites.cfg') or definedIn.startswith("favfolders/"):
                ...
                if lItem['title'] != "Favourites":
                        # Add to favourites
                        contextMenuItem = createContextMenuItem('Add to PGLiveTV favourites', Mode.ADDTOFAVOURITES, codedItem)
                        contextMenuItems.append(contextMenuItem)
    ...
    def update(self):
        ...
        def doUpdates(typeName, updates):
            ...
            head = "PGLiveTV Updates - %s" % typeName
        ...
        if count == 0:
            common.showNotification('PGLiveTV', common.translate(30273))
            return
        else:
    ...
    def queueAllVideos(self, item):
        dia = DialogProgress()
        dia.create('PGLiveTV', 'Get videos...' + item['title'])
        dia.update(0)
    ...
    def run(self, argv=None):
        ...
        self.addon = Addon('plugin.video.PGLiveTV', argv)

        common.log('PGLiveTV running')
        ...
        try:
            ...
            common.showError('Error running PGLiveTV')
            common.log('Error running PGLiveTV. Reason:' + str(e))
